...
describe('CurrencyFormat as text', () => {
  it('should format numbers to currency', () => {
    const wrapper = shallow(<CurrencyFormat value={2456981} displayType={'text'} thousandSeparator={true} prefix={'$'} />);
    expect(wrapper.find('span').text()).toEqual('$2,456,981');
  });

  it('should format as given format', () => {
    const wrapper = shallow(<CurrencyFormat value={4111111111111111} displayType={'text'} format="#### #### #### ####" />);
  ...
  it('should format as given format when input length is less than format length', () => {
    const wrapper = shallow(<CurrencyFormat value="41111111111111" displayType={'text'} format="#### #### #### ####" />);
    expect(wrapper.find('span').text()).toEqual('4111 1111 1111 11  ');
  });

  it('should format as given format with mask', () => {
    const wrapper = shallow(<CurrencyFormat value="41111111111111" displayType={'text'} format="#### #### #### ####" mask="_" />);
    expect(wrapper.find('span').text()).toEqual('4111 1111 1111 11__');
  });

  it('should limit decimal scale to given value', () => {
    const wrapper = shallow(<CurrencyFormat value={4111.344} displayType={'text'} decimalScale={2}/>);
  ...
  it('it should add zeros if fixedDecimalScale is provided', () => {
    const wrapper = shallow(<CurrencyFormat value="4111.11" displayType={'text'} decimalScale={4} fixedDecimalScale={true}/>);
  ...
  it('should accept custom renderText method', () => {
    const wrapper = shallow(<CurrencyFormat value="4111.11" thousandSeparator="," renderText={value => <div>{value}</div>} displayType={'text'} />);
